...
class GestionBibliotecaImpl extends GestionBibliotecaPOA {
    ...
    public boolean agregarLibro(Libro nuevoLibro) {
        if (!libros.containsKey(nuevoLibro.ISBN)) {
            libros.put(nuevoLibro.ISBN, nuevoLibro);
            return true;
        }
        return false;
    }
    ...
    public Libro[] buscarPorAutor(String autor) {
        return libros.values().stream().filter(libro -> libro.autor.equalsIgnoreCase(autor)).toArray(Libro[]::new);
    }
    @Override
